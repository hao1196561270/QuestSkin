...
| **精简右键菜单** | 5 项：焦点 / 打开任务细节 / 打开任务地图 / 追踪-取消追踪 / 放弃（战斗中置灰防 taint），`MenuUtil` 优先 + 自绘兜底 |
...
- **打开任务详情**：点击任意任务块主体
- **右键菜单**：右键任务块 → 焦点 / 打开任务细节 / 打开任务地图 / 追踪-取消追踪 / 放弃（战斗中置灰）
...
- **v0.0.1** — 预览版：地图动态 Ticker（`OnShow` 1.0s 轮询 / `OnHide` 5s 后取消，替代常驻 0.4s）、超追踪高亮（◆ 超追踪 / ◇ 普通追踪，标题与指示线联动）、精简 5 项右键菜单（`MenuUtil` + 自绘，移除 `UIDropDownMenu`/Share/BtW/Wowhead）、成就追踪入列（`C_ContentTracking` 优先，已完成过滤）、放弃按钮战斗中置灰（`InCombatLockdown`）、`MigrateDB version=1`、发布流水线（`.pkgmeta` + `release.yml`）
